...
function isOppositeDirection(
  ...
  const isVerticalBacktrack = (dirFrom === "down" && dirTo === "up") || (dirFrom === "up" && dirTo === "down");
  const isHorizontalBacktrack = (dirFrom === "left" && dirTo === "right") || (dirFrom === "right" && dirTo === "left");
...
export function renderPath(
  ...
  for (let i = 0; i < lastIndexToDraw; i++) {
    ...
    const gradientValue = pathLength > 1 
      ? (pathLength - 1 - i) / (pathLength - 1)
      : 1;
    ...
    if (dirFrom && dirTo && dirFrom !== dirTo && !oppositeDir) {
      ...
      let rotation = 0;
      if (dirFrom === "down") rotation = 0;           // down down up
      else if (dirFrom === "up") rotation = 180;      // up up down
      else if (dirFrom === "left") rotation = 90;     // left left right
...
export function renderPathArrow(
  ...
  if (path.length > 0 && state.status === "idle") {
    ...
    if (path.length > 1) {
      ...
      if (dir === "right") arrowRotation = 270;      // right
      else if (dir === "left") arrowRotation = 90;   // left
      else if (dir === "down") arrowRotation = 0;    // down
